...
We have generated 11 feature models. We can observe in
Table~\ref{execution_times} that the results are similar in each
model. That is, most of  the features require between 80 and 4599 milliseconds to be processed.
...
time for processing the model is considerably reduced. Hence, the
...
and the cost extension \fodaPAc~\cite{clc16} and, therefore, large models containing
an elevated number of features are processed more efficiently. Although these
previous implementations also allow to calculate all the valid products of a model, the
required processing time to accomplish this task is elevated, making the
processing of large models unfeasible. Alternatively, the implementation of the denotational
semantic \fodaPA~\cite{acl13} also calculates the satisfactibility of a
model, that is, checks if the model contains, at least, a valid product. In this case,
