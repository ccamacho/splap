...
mainly caused by the inherent noise of the node where the experiment is launched (e.g. disk latencies, operating
...
The model used in the experiments has been executed 30 times, providing similar results. That is, the
major part of the features require between 15 and 20 milliseconds to be processed, while there is a small
portion of them requiring a processing time between 20 and 36 milliseconds.
The standard deviation of the experiment is 3.0357.

For the sake of the article readability we only display the first 10 executions in
Table~\ref{execution_times}, it displays partially the results to
show that on each execution the algorithm behaves similarly.

\begin{table}[h]
	\centering
	\begin{tabular}{|c|c|c|c|c|}
		\hline
		\textbf{Execution} & \textbf{Maximum} &  \textbf{Minimum} &  \textbf{Average} &  \textbf{Standard deviation} \\ \hline
...
Specifically, we are interested in comparing the performance between the implementation 
of the probabilistic extension and the implementation of the denotational semantic from 
\fodaPA~\cite{acl13}.
