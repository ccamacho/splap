...
% First, we check the satisfiability of the generated model. 
% In Subsection~\ref{subsection:satifiability:module} some results 
% presented the scalability of this approach are shown. Then, 
% after checking that the model is satisfiable in Subsection~\ref{subsection:denotational:semantics}
% we apply the denotational semantic rules in this model in order to show the complete set of products, and to calculate the warnings and errors.
...
This module computes the satisfiability of an \SPL\ by
...
The experiments were run with a number of different features ranging from $50$ to $300$.
%Each experiment was executed 50 times. 
...
\begin{figure}[t]
	...
	\begin{tabular}{p{0.5\hsize}p{0.5\hsize}}
		...
		\begin{tabular}[t]{ll}
			\parbox[t]{0.8\hsize}{
				$ $\\
				$\feature{S}$ : Chef Server \\
				$\feature{R}$ : Roles \\
				$\feature{E}$ : Environments \\
				$\feature{N}$ : Nodes \\
				$\feature{P}$ : Production       \\        
				$\feature{T}$ : Test  \\
				$\feature{W}$ : Web server \\
				$\feature{D}$ : Database Server  \\      
				$\feature{M}$ : Monitoring  service \\
				$\feature{AP}$ : Apache        \\     
				$\feature{NG}$ : Nginx      \\       
				$\feature{MS}$ : MySQL        \\     
				$\feature{SS}$ : SQLServer      \\   
				$\feature{NA}$ : Nagios        \\     
				$\feature{NE}$ : New Relic  \\
				$\feature{N1}$ : Node 1        \\     
				$\feature{N2}$ : Node 2    
			}
	...
	\hrule
	\caption{\FODA\ to \fodaPA\ from~\cite{clc16}.}
	\label{figure:chef:term}
\end{figure}
...
We have tested our implementation in an 8-node cluster, where each node has
the following configuration:
\begin{description}
	\item[Operating System:] Ubuntu 14.04-trusty
	\item[RAM:] 4GB
