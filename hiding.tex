...
\bprop
  ...
  $$\begin{array}{rll} 
  \semdenp{\checkmark\hide\calA}&=&\semdenp{\checkmark}\\
  ...
  \semdenp{(\ofA;_rP)\hide\calA}&=&
      ...
      \end{cases}\\
  \semdenp{(P\choice_P Q)\hide\calA}&=&\semdenp{(P\hide\calA)\choice_P (Q\hide\calA)}\\
  \semdenp{(P\paral Q)\hide\calA}&=&\semdenp{(P\hide\calA)\paral (Q\hide\calA)}\\
  \multicolumn{3}{l}{\mathrm{If\ } \fA,\fB\not\in\calA \mathrm{\ then\ }
  ...
  \end{array}$$
\mncomment{Luis, faltan casos: que ocurre si $A$ y/o $B$ pertenecen al conjunto ocultado??}
%  \begin{proof}
