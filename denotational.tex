...
\label{sec:stat:den}
\mncomment{Comprobad que he interpretado bien lo que ponÌa en los itemizes originales}
...
%\todo{Explicar el modelo con m√°s detalle,  y el motivo por el que se necesitan multiconjnutos}
...
We define the semantic domain $\calM$ as the largest set $\calM\subseteq\calP(\calP(\calF)\times
  ...
  conditions hold:
  \begin{itemize}
  \item \mncomment{He quitado esta condiciÛn porque es redundante!}
  %$M\subseteq \calP(\calF)\times (0,1]$
  \item If $(P,q)\in M$ and $(P,r)\in M$ then $q=r$.
  \item $0\leq \sum \lbag q \ | \ \exits P: \sum_{(P,q)\in M\rbag \leq 1$.
  \end{itemize}
%\edfn
...
%\bdfn
Let $M \in \cal$. \mncomment{Pone que es un multiset???!!!! Si es un objeto sem·ntico, es un set, no??} 
We define the operator $\accum$ as follows:
  $$\accum(M) = \left\{(P,p)\ \left| \ p=\sum_{(P,q)\in M}q\ ,\ p>0\right \right\}$$
\edfn
...
\bprop\label{prp:domain:prob}
  ...
  las caracter√≠sticas $\feature{A},\feature{B}\in\calF$, entonces:
  \begin{itemize}
