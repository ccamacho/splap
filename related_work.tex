...
depending on how the models are computed and how the models are represented, all this varies
depending on the information to be generated~\cite{dpcslsh17}.
This analysis makes the whole process of studding product lines a computational complex task.

In particular there exist mathematical verification models~\cite{vk13}
allowing the behavior representation of the products of a product line using models
like discrete time Markov chain families, allowing
to represent the probabilistic behavior of all
the products in the product line but its implementation does not
provides a literal translation between the already know variability models like FODA
and they also do not provide implementation over their practical uses, although,
they show analysis techniques that once implemented might have
a drastic impact in the effort to compute variability models.

In previous years the studies relating the analysis of variability models and
their practical applications with realistic use cases ~\cite{jhf11, jhfes12}
demonstrate that those uses cases do not describe such complex models, thus, they
...
side its described that those models for defining products
...
Based on the previous work in the literature we define the following research questions:

\begin{itemize}
	\item Is it possible to translate current variability
	models to support probabilistic information?.
	\item Is it possible to describe a formal framework
	that translates the current variability models
	to probabilistic methods?.
	\item What is the impact of applying probabilistic analysis
	methods to current variability models like FODA?.
\end{itemize}
